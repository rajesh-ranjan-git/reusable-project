...
export const assertSocketTargetUser = async (socket, targetUserId) => {
  if (!targetUserId || targetUserId.toString() === socket.data.userId?.toString()) {
    socket.emit("error", {
      event: socket._lastEvent,
      message: "Please provide a valid target user.",
    });
    return false;
  }

  const targetUser = await User.findById(targetUserId).select("_id status");
...
export const initializeSocket = (server) => {
  const io = new Server(server, {
    path: MODE === "production" ? "/server/socket.io" : "/socket.io",
    cors: {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join-chat", async ({ targetUserId }) => {
      try {
        const isValidTarget = await assertSocketTargetUser(socket, targetUserId);

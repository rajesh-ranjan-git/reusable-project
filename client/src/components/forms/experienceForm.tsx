...
type FieldErrors = Partial<Record<keyof Experience, string>>;

const validateEntry = (exp: Experience): FieldErrors => {
...
const ExperienceForm = ({
  ...
  const [entries, setEntries] = useState<Experience[]>(
    initialData.length > 0 ? initialData : [emptyExperience()],
  );
  ...
  useEffect(() => {
    if (isOpen) {
      setEntries(initialData.length > 0 ? initialData : [emptyExperience()]);
      setErrors([]);
    }
  }, [isOpen]);
